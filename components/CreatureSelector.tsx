...
interface CreatureSelectorProps {
  ...
  onChange: (creatureId: number) => void;
  isDisabled: boolean;
  onShuffle: () => void;
}

const CreatureSelector: React.FC<CreatureSelectorProps> = ({ selectedCreatureId, onChange, isDisabled, onShuffle }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const wrapperRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  const selectedCreature = CREATURES.find(l => l.id === selectedCreatureId) || CREATURES[0];
  ...
  const filteredCreatures = useMemo(() => {
    ...
    return CREATURES.filter(creature =>
      creature.name.toLowerCase().includes(lowercasedFilter)
    );
  }, [searchTerm]);

  const handleSelect = (creatureId: number) => {
    onChange(creatureId);
    setIsOpen(false);
  };
    ...
    <div className="relative font-roboto-mono">
      ...
      <div className="flex gap-2">
        <div ref={wrapperRef} className="flex-grow relative">
          ...
          >
            <span className="block truncate uppercase text-sm tracking-wider">
            ...
            <div className="absolute z-20 mt-1 w-full bg-gray-900 shadow-lg rounded-md ring-2 ring-black ring-opacity-50 border-2 border-gray-700">
              <div className="p-2">
                  ...
                  <ul
                      ...
                      tabIndex={-1}
                      role="listbox"
                      aria-labelledby="creature-select-button"
                  >
                      {filteredCreatures.map((creature) => (
                          ...
                          role="option"
                          aria-selected={creature.id === selectedCreatureId}
                          onClick={() => handleSelect(creature.id)}
                          >
                          <div className="flex items-center">
                              <span className={`font-normal ml-3 block truncate text-xs uppercase ${creature.id === selectedCreatureId ? 'font-semibold text-white' : ''}`}>
                              {creature.name}
                              </span>
                          </div>
        ...
        <button
            ...
            title="Randomize Vessel"
            aria-label="Randomize Vessel"
            >
